...
import com.buggysofts.clvl.ILicenseResultListener;
import com.buggysofts.clvl.ILicensingService;
 ...
 * <p>
 * The LicenseChecker is configured via a {@link com.buggysofts.clvl.GPSLPRules} which contains the logic to determine
 ...
 */
public class GPSLVerifier implements ServiceConnection {
    ...
    private final Set<com.buggysofts.clvl.GPSLLVerifier> mChecksInProgress = new HashSet<com.buggysofts.clvl.GPSLLVerifier>();
    private final Queue<com.buggysofts.clvl.GPSLLVerifier> mPendingChecks = new LinkedList<com.buggysofts.clvl.GPSLLVerifier>();

    /**
     * @param context          a Context
     * @param GPSLPRules       implementation of Policy
     * @param encodedPublicKey Base64-encoded RSA public key
     * @throws IllegalArgumentException if encodedPublicKey is invalid
     */
    public GPSLVerifier(Context context, com.buggysofts.clvl.GPSLPRules GPSLPRules, String encodedPublicKey) {
        mContext = context;
     ...
     * @param callback
     */
    public synchronized void checkAccess(com.buggysofts.clvl.GPSLCListener callback) {
        // If we have a valid recent GPSLAllowed response, we can skip asking
        // Market.
        if (mGPSLPRules.allowAccess()) {
            Log.i(TAG, "Using cached license response");
            callback.GPSLValidated(com.buggysofts.clvl.GPSLPRules.GPSLAllowed);
        } else {
            com.buggysofts.clvl.GPSLLVerifier validator =
                new com.buggysofts.clvl.GPSLLVerifier(
                    mContext,
                    mGPSLPRules,
                    new com.buggysofts.clvl.NullGPSLDShrinker(),
                    callback,
                    generateNonce(),
                    mPackageName,
                    mVersionCode
            ...
            if (mService == null) {
                ...
                try {
                    ...
                    callback.applicationError(com.buggysofts.clvl.GPSLCListener.ERROR_MISSING_PERMISSION);
                } catch (Base64DecoderException e) {
                    e.printStackTrace();
                }
            } else {
    ...
    public void followLastLicensingUrl(Context context) {
        String licensingUrl = mGPSLPRules.getLicensingUrl();
    ...
    private synchronized void finishCheck(com.buggysofts.clvl.GPSLLVerifier validator) {
    ...
    private class ResultListener extends ILicenseResultListener.Stub {
        private final com.buggysofts.clvl.GPSLLVerifier mValidator;
        private Runnable mOnTimeout;

        public ResultListener(com.buggysofts.clvl.GPSLLVerifier validator) {
     ...
     * timeouts.
     */
    private synchronized void handleServiceConnectionError(com.buggysofts.clvl.GPSLLVerifier validator) {
        mGPSLPRules.processServerResponse(com.buggysofts.clvl.GPSLPRules.RETRY, null);

        if (mGPSLPRules.allowAccess()) {
            validator.getCallback().GPSLValidated(com.buggysofts.clvl.GPSLPRules.RETRY);
        } else {
            validator.getCallback().GPSLDValidated(com.buggysofts.clvl.GPSLPRules.RETRY);
        }
    }
    ...
    /**
     * Inform the library that the context is about_icon to be destroyed, so that any open connections
     * can be cleaned up.
     * <p>
