 ...
 * error retry count and a URL for restoring app access in unGPSLAllowed cases.
 * <p>
 ...
 */
public class APKExpansionGPSLPRules implements com.buggysofts.clvl.GPSLPRules {

    private static final String TAG = "APKExpansionPolicy";
    private static final String PREFS_FILE = "com.buggysofts.clvl.APKExpansionPolicy";
    ...
    private com.buggysofts.clvl.PreferenceObfuscator mPreferences;
     ...
     * @param randomizer An obfuscator to be used with preferences.
     */
    public APKExpansionGPSLPRules(Context context, com.buggysofts.clvl.Randomizer randomizer) {
        // Import old values
        SharedPreferences sp = context.getSharedPreferences(PREFS_FILE, Context.MODE_PRIVATE);
        mPreferences = new com.buggysofts.clvl.PreferenceObfuscator(sp, randomizer);
        mLastResponse = Integer.parseInt(
                mPreferences.getString(PREF_LAST_RESPONSE, Integer.toString(com.buggysofts.clvl.GPSLPRules.RETRY)));
    ...
    public void resetPolicy() {
        mPreferences.putString(PREF_LAST_RESPONSE, Integer.toString(com.buggysofts.clvl.GPSLPRules.RETRY));
     ...
     * <li>LU: a deep link URL that can enable access for unGPSLAllowed apps (e.g.
    ...
    public void processServerResponse(int response,
        ...
        if (response != com.buggysofts.clvl.GPSLPRules.RETRY) {
            setRetryCount(0);
        } else {
        ...
        if (response == com.buggysofts.clvl.GPSLPRules.GPSLAllowed) {
            mLastResponse = response;
            // Reset the licensing URL since it is only applicable for NOT_GPSLAllowed responses.
        ...
        } else if (response == com.buggysofts.clvl.GPSLPRules.NOT_GPSLAllowed) {
     ...
     * <ol>
     * <li>a GPSLAllowed response was received within the validity period
    ...
    public boolean allowAccess() {
        ...
        if (mLastResponse == com.buggysofts.clvl.GPSLPRules.GPSLAllowed) {
            // Check if the GPSLAllowed response occurred within the validity
            // timeout.
            // Cached GPSLAllowed response is still valid.
            return ts <= mValidityTimestamp;
        } else if (mLastResponse == com.buggysofts.clvl.GPSLPRules.RETRY &&
